...
def sidebar_filters():
    ...
    f_desde = st.sidebar.date_input("Desde", value=st.session_state.filters.get("fecha_desde", default_from))
    f_hasta = st.sidebar.date_input("Hasta", value=st.session_state.filters.get("fecha_hasta", default_to))

    programas = DATA.list_programas()
    prog_map = {r["nombre"]: int(r["id"]) for _, r in programas.iterrows()} if not programas.empty else {}
    prog_sel = st.sidebar.selectbox("Programa", options=["(Todos)"] + list(prog_map.keys()))
    ...
    conv_sel = st.sidebar.selectbox("Convenio", options=["(Todos)"] + list(conv_map.keys()))
    ...
    prof_sel = st.sidebar.selectbox("Profesor", options=["(Todos)"] + list(prof_map.keys()))
    ...
    inst_sel = st.sidebar.selectbox("Institución", options=["(Todas)"] + list(inst_map.keys()))
    institucion_id = inst_map.get(inst_sel)

    departamento = st.sidebar.text_input("Departamento (exacto)", value=st.session_state.filters.get("departamento", ""))
    municipio = st.sidebar.text_input("Municipio (exacto)", value=st.session_state.filters.get("municipio", ""))
...
def ui_cargar_datos(auth_email: Optional[str]):
    ...
    programas = DATA.list_programas()
    prog_map = {r["nombre"]: int(r["id"]) for _, r in programas.iterrows()} if not programas.empty else {}
    prog_sel = c1.selectbox("Programa", options=list(prog_map.keys()) if prog_map else [], index=0 if prog_map else None, placeholder="Selecciona o crea uno en Configuración")
    programa_id = prog_map.get(prog_sel)

    convenios = DATA.list_convenios(programa_id=programa_id) if programa_id else pd.DataFrame()
    conv_map = {r["nombre"]: int(r["id"]) for _, r in convenios.iterrows()} if not convenios.empty else {}
    conv_sel = c1.selectbox("Convenio", options=list(conv_map.keys()) if conv_map else [], index=0 if conv_map else None)
    convenio_id = conv_map.get(conv_sel)

    profesores = DATA.list_profesores(programa_id=programa_id, convenio_id=convenio_id)
    prof_map = {r["nombre"]: int(r["id"]) for _, r in profesores.iterrows()} if not profesores.empty else {}
    prof_sel = c2.selectbox("Profesor", options=list(prof_map.keys()) if prof_map else [], index=0 if prof_map else None)
    profesor_id = prof_map.get(prof_sel)

    instituciones = DATA.list_instituciones()
    inst_map = {f"{r['nombre']} ({r['municipio'] or ''}-{r['departamento'] or ''})": int(r["id"]) for _, r in instituciones.iterrows()} if not instituciones.empty else {}
    inst_sel = c2.selectbox("Institución", options=list(inst_map.keys()) if inst_map else [], index=0 if inst_map else None)
    institucion_id = inst_map.get(inst_sel)

    # Geografía (permite sobreescribir lo de la institución si aplica)
    c4, c5, c6, c7 = st.columns([1, 1, 1, 1])
    fecha = c4.date_input("Fecha", value=date.today())
    localidad = c5.text_input("Localidad (opcional)")
    municipio = c6.text_input("Municipio (opcional)")
    departamento = c7.text_input("Departamento (opcional)")

    c8, c9, c10 = st.columns([1, 1, 2])
    programados = c8.number_input("Pacientes programados", min_value=0, step=1)
    atendidos = c9.number_input("Pacientes atendidos", min_value=0, step=1)
    observaciones = c10.text_area("Observaciones", placeholder="Notas relevantes de la jornada")
...
def ui_registros():
    ...
    id_sel = c1.number_input("ID de registro", min_value=1, step=1)
...
def ui_configuracion():
    ...
    with tabs[0]:
        ...
        p_nombre = c1.text_input("Nombre del programa")
    ...
    with tabs[1]:
        ...
        cv_prog = c1.selectbox("Programa", options=list(prog_map.keys()) if prog_map else [])
        cv_nombre = c2.text_input("Nombre del convenio")
    ...
    with tabs[2]:
        ...
        i_nombre = c1.text_input("Nombre institución")
        i_localidad = c2.text_input("Localidad")
        i_municipio = c3.text_input("Municipio")
        i_departamento = c4.text_input("Departamento")
    ...
    with tabs[3]:
        ...
        conv_map = {r["nombre"]: int(r["id"]) for _, r in convenios.iterrows()} if not convenios.empty else {}
        c1, c2, c3, c4, c5 = st.columns([2, 1.2, 1.4, 1.2, 1])
        f_nombre = c1.text_input("Nombre profesor/a")
        f_doc = c2.text_input("Documento (opcional)")
        f_email = c3.text_input("Email (opcional)")
        f_prog = c4.selectbox("Programa", options=list(prog_map.keys()) if prog_map else [])
        f_conv = c4.selectbox("Convenio", options=list(conv_map.keys()) if conv_map else [], key="f_conv")
        if c5.button("Agregar profesor/a", use_container_width=True, disabled=not f_nombre.strip()):
